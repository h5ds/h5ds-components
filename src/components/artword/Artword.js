  ...
  render() {
    ...
    switch (type) {
      ...
      case 'jiaXin':
        ...
        upStyle.WebkitTextFillColor = `${size}px ${colors[0]}`;
        downStyle.WebkitTextStroke = `${size}px ${colors[1]}`;
        otherStyle = { zIndex: 99, WebkitTextStroke: `${size + size / 2}px ${colors[0]}` };
        break;
      case 'yinYing':
        upStyle.color = colors[0];
    ...
    // 判断是否有dangerouslySetInnerHTML参数
    if (shtml) {
      return (
        <div style={style} className={cName.join(' ')}>
          <div style={upStyle} className="h5ds-artword-up" dangerouslySetInnerHTML={{ __html: shtml }} />
          <div style={downStyle} className="h5ds-artword-down" dangerouslySetInnerHTML={{ __html: shtml }} />
          {otherStyle ? (
            <div style={otherStyle} className="h5ds-artword-up" dangerouslySetInnerHTML={{ __html: shtml }} />
          ) : null}
        </div>
      );
    } else {
